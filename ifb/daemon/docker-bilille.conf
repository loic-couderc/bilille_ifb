...
pre-start script
    ...
    if [ ! -d "{{ bilille.output_dir }}/antismash/example" ]; then
        /bin/mkdir -p "{{ bilille.output_dir }}/antismash/example"
    fi
    /bin/mount -o bind "{{ bilille.install_dir }}/antismash/example" "{{ bilille.output_dir }}/antismash/example"
